...
class SquareRenderer implements GLSurfaceView.Renderer {
	...
	private Vector2D cannonPos = new Vector2D(2.4f, 0.5f);
	...
	public SquareRenderer(boolean useTranslucentBackground, Context c) {
		...
		mCannonBall[0] = new CannonBall(vertices2);
	}
	...
	public void onDrawFrame(GL10 gl) {
		...
		gl.glLoadIdentity();
		gl.glRotatef(mTransAngle, 0, 0, 1);
		mSquare[0].draw(gl);
	...
	public void update(float x, float y, GLSurfaceView Gview, MotionEvent event) {
		...
		touchPos.x = (x / (float) Gview.getWidth()) * FRUSTUM_WIDTH;
		touchPos.y = (1 - y / (float) Gview.getHeight()) * FRUSTUM_HEIGHT;
...
//		ballVelocity.add((float) (gravity.x * 0.5), (float) (gravity.y * 0.5));
//		ballPos.add((float) (ballVelocity.x * 0.5),
//				(float) (ballVelocity.y * 0.5));
